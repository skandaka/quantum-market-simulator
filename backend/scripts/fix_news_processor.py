...
class NewsProcessor:
    ...
    async def _enrich_by_source(self, news_input: NewsInput) -> Dict[str, Any]:
        ...
        if news_input.source_type == NewsSourceType.TWEET:
            ...
            try:
                article_data = await self._fetch_article_simple(str(news_input.source_url))
                enriched.update(article_data)
    ...
    async def _fetch_article_simple(self, url: str) -> Dict[str, Any]:
        """Fetch and parse article using BeautifulSoup"""
        if not self.session:
            self.session = aiohttp.ClientSession()
    ...
    def _extract_financial_keywords(self, text: str) -> Dict[str, List[str]]:
